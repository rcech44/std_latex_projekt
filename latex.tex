...
\usepackage[a4paper, left=20mm, right=30mm, bottom=35mm, top=30mm]{geometry}
\usepackage[utf8]{inputenc}
\usepackage[czech]{babel}
\usepackage{lmodern}
%\usepackage[cp1250]{inputenc} % pro win1250
\usepackage{fancyhdr}
%\usepackage{tabto} -- nakonec nevyuzito
\usepackage{cmap}
\usepackage{siunitx}
\usepackage{xcolor}
\usepackage{amsmath}
\usepackage{multirow}
%\usepackage{caption} -- neni potreba
\usepackage{graphicx}
...
\definecolor{strings}{rgb}{0.7,0.13,0.13}
\definecolor{keywords}{rgb}{0.145,0.6,0.698}
\definecolor{identifier}{rgb}{0.145,0.6,0.698}
...
\newenvironment{algoritmus}[1][htb]
  {\renewcommand{\algorithmcfname}{Algoritmus}  % -- Prepsani nazvu algoritmu
   \begin{algorithm}[#1]%
  }{\end{algorithm}}

\def\blankpage{%        %% definovani prazdne stranky, workaround
      \clearpage%
      \null%
      \clearpage}
...
\pagestyle{fancy}
\fancyhf{} % nastaveni prazdnych zahlavi a zapati
\renewcommand{\headrulewidth}{0pt}
\fancyhead[RO]{Sazba technických dokumentů - semestrální projekt}
\fancyhead[LE]{Sazba technických dokumentů - semestrální projekt}
\fancyfoot[LE]{\thepage}
\fancyfoot[RE]{Radomír Čech, CEC0144}
\fancyfoot[RO]{\thepage}
\fancyfoot[LO]{Radomír Čech, CEC0144}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=60mm]{pics/Fig003.pdf}
...
\begin{table}[t]
    ...
    \toprule
     & \multicolumn{6}{c}{Algoritums 1} \\
    \cmidrule(lr){2-7}
      & \multicolumn{2}{c}{$\alpha$} &  \multicolumn{2}{c}{$\beta$} & \multicolumn{2}{c}{$\gamma$} & \multicolumn{2}{c}{$\delta$} & \multicolumn{2}{c}{$\chi$}\\
    \midrule
...
\begin{figure}[t]
    \centering
    \includegraphics[width=60mm]{pics/Fig004.pdf}
...
\For{i $\leftarrow$  length(a)}{
  \eIf{a[i] \% 2 == 0}{
   sum\_1 += a[i] + b[i]; \Comment{Komentář 1}\\
   }{
   sum\_2 += b[i] - a[i]\;
  }
 }
...
%\lstinputlisting[language=Python]{SourceCode.py}   -- taky moznost
